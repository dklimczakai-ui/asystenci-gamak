...
| **NEW: A1** | Audyt logów CyberFolks: FTP login attempts w okresie 2026-05-04 → 05-05 — czy w ekspozycji 18h ktoś nieautoryzowany próbował FTP | ⏳ TAK Daniela (~5 min, panel CF lub support) — **walkthrough gotowy:** `audits/2026-05-05_A1_A2_walkthrough.md` § A1 (deadline 2026-05-07 22:00) | R1 incident follow-up |
| **NEW: A2** | github.com/settings/applications: czy są nieoczekiwane OAuth tokens z scope `repo` | ⏳ TAK Daniela (~3 min, manual) — **walkthrough gotowy:** `audits/2026-05-05_A1_A2_walkthrough.md` § A2 (deadline 2026-05-07 22:00) | R1 incident follow-up |
| ~~**NEW: A4**~~ | ~~Skan `beauty/.claude/settings.local.json`~~ | ✅ **WYKONANE 2026-05-05** (yolo session) — plik 7 linii, 1 allowlist `Bash(aws sts *)`, ZERO bash history, ZERO sekretów. CLEAN. |
| ~~**NEW: A6**~~ | ~~Procedura GITHUB.md — rozszerzenie sekcji "Procedura kryzysowa" o pre-flight checklist filter-repo (10-krokowa, R17 kandydat)~~ | ✅ **WYKONANE 2026-05-05** (yolo session) — `GITHUB.md` § Krok 4.0 + 4.1 + 4.2 + 4.3 dopisane, 10 kroków pre-flight checklist + audit raport sections. |
| ~~**NEW: A7**~~ | ~~Pre-commit hook (TruffleHog/git-secrets)~~ | ✅ **v1.0 WDROŻONE 2026-05-05** (yolo session) — bash script `costsec/scripts/git-hooks/pre-commit` (filename blacklist + 12 secret patterns + R1 incident patterns + PII PL telefon), aktywowany przez `git config core.hooksPath`. Dług R6 do v1.1 TruffleHog: trigger = pierwszy false negative który v1.0 puścił. |
| ~~**NEW: A8**~~ | ~~Skan `deploy.zip.bak`~~ | ✅ **WYKONANE 2026-05-05** (yolo session) — plik już usunięty (rm -f w settings.local.json allowlist). Brak. |
| ~~**NEW: A9**~~ | ~~`mail/docs/CHANGELOG.md` sanitize PII zewnętrznych~~ | ✅ **WYKONANE 2026-05-05** (yolo session) — sanitize w workdir: `Tutu-Nexnovo` → `<dostawca-led-cn-1>` (5×), `Mds Display` → `<dostawca-led-cn-2>` (4×), `Peter` standalone → `<klient-zagraniczny-engo>` (4×). Łącznie ~13 wystąpień. **Wewnętrzne (Wiesław Klimczak, jego osobiste maile, "tata Daniela") zostawione** — wymagają osobnej decyzji Daniela (publiczna funkcja PFU writer GAMAK vs prywatność rodzinna). Filter-repo NIE wykonane — wymaga osobnego TAK Daniela (R17 destrukcyjny, ekspozycja PRIVATE niska). |
| ~~**NEW: RODO Art. 33**~~ | ~~Decyzja czy notyfikacja UODO 72h~~ | ⏳ Daniel decyzja (deadline 2026-05-07 22:00) — **decision support memo gotowy:** `audits/2026-05-05_RODO_decision.md`. Rekomendacja CTO: NIE zgłaszać (warunek: A2 czyste). Template formularza UODO + szablon `decyzje.md` w memo. |
| ~~**NEW: R15-R18**~~ | ~~Zatwierdzenie kandydatów R15+R16+R17+R18 do twardych zasad~~ | ✅ **ZATWIERDZONE 2026-05-05** (yolo session, trigger Daniela `/yolo r15-r18`) — status w `ZASADY.md` § Część 4 § Kandydaci zmieniony na "TWARDA ZASADA — obowiązuje". Pre-commit hook v1.0 już egzekwuje R15+R16+R18. R17 udokumentowany w `GITHUB.md` § Krok 4. Fizyczne przesunięcie do Część 1 ZASADY.md = osobna sesja (refactor numeracji). |
