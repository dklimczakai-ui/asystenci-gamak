...
- **R15 — `.claude/settings.local.json` + bash command history NIGDY tracked w repo** *(✅ ZATWIERDZONE 2026-05-05 jako TWARDA ZASADA — obowiązuje, trigger: osobny TAK Daniela `/yolo r15-r18` po R1 incident response)*
  - **Treść:** Plik `.claude/settings.local.json` (Claude Code per-project settings) zawiera **literalny zapis bash command history** wykonanych przez agenta. Każda komenda z hasłem/tokenem/kluczem w argumentach (curl FTP, mysql -p, kubectl --token, aws cli z literalnym secret) zostaje tam ZAPISANA. Plik MUSI być w `.gitignore` od dnia 0 każdego nowego repo. Nawet w PRIVATE repo. Dotyczy też wszystkich `**/settings.local.json`, `.claude/projects/`, `~/.claude/`.
  - **Dlaczego chroni biznes:** Incydent R1 z 2026-05-05 — hasło FTP CyberFolks wyciekło do repo PRIVATE przez `gamak/.claude/settings.local.json` (~30 wystąpień bash history z `curl ftp://... -u xkhvbgqqku:<hasło>`). Plik tracked od Initial commit (2026-05-04 rano), wykryto wieczorem, naprawione 2026-05-05 przez filter-repo + force push + rotacja hasła. Bez R15 ten wzorzec się powtórzy z każdym nowym repo Daniela.
  - **Jak sprawdzamy:** Pre-commit hook (TruffleHog / git-secrets / custom regex) skanuje `.claude/settings.local.json` (oraz `**/settings.local.json`) przed każdym commit. Plus G0 (przed Initial Commit nowego repo): `git ls-files | grep -E "settings\.local|\\.claude"` — jeśli zwraca cokolwiek poza wzorcami `.claude/extensions/` i podobnymi safe — STOP i fix `.gitignore`. Audyt monthly: `find . -name 'settings.local.json' -not -path '*/node_modules/*'` + `git ls-files` cross-check.
  - **Trigger aktywacji jako twarda R15:** drugi wyciek tego samego typu (np. `.claude/settings.local.json` w jakimkolwiek innym repo Daniela) ALBO osobne TAK Daniela po review tego incydentu.

- **R16 — Test data dla systemów z PII = mock-from-day-0** *(✅ ZATWIERDZONE 2026-05-05 jako TWARDA ZASADA — obowiązuje, trigger: osobny TAK Daniela `/yolo r15-r18` po R1 incident response)*
  - **Treść:** Każdy plik testowy (`test_*.json`, `*_test.json`, `inbox_test.*`, `payload_test.*`, fixtures) dla Lambdy / serwisu obsługującego PII klientów musi być **mock-from-day-0** — od pierwszego utworzenia. Konwencje obowiązkowe: emaile = `<placeholder>@example.com`, imiona = `<imię>`, telefony = `+48 XXX XXX XXX`, kwoty = `<XXX_XXX>`, nazwy firm = `<Firma Sp. z o.o.>`, ID = `mock-uuid-NNNN`. Jeśli developer potrzebuje real data debug — używa pliku z sufix `_real.json` (gitignored przez sekcję 9 .gitignore) lub folder `_local/` (gitignored). NIGDY nie commituje real data nawet "tymczasowo".
  - **Dlaczego chroni biznes:** Incydent R1 z 2026-05-05 — `inbox_test.json` zawierał 21 realnych draftów AI z PII zewnętrznych klientów (3 klienci zagraniczni + 3 klienci PL z firmami budowlanymi/biuro) + wewnętrzne notatki strategiczne (decyzje przetargowe, kwoty wadiów, ceny ofertowe). Plik powstał jako "test data" dla mail-agent-api endpoint, ale wygoda real data debug skończyła się na commit do PRIVATE repo. Naruszenie R5 + V2. Bez R16 wzorzec się powtórzy w każdym nowym test pliku Lambdy.
  - **Jak sprawdzamy:** Pre-commit hook regex skan: `grep -E "@(?!example\.com|placeholder)|\\+48 [0-9]" test_*.json`. Plus każda karta systemu w `SYSTEMY.md` z polem 6 (Dane i sekrety) wymieniającym test data — sekcja 14 musi potwierdzić "test data: mock-from-day-0 zgodnie z R16". Audyt monthly: skan wszystkich `test_*.json` pod kątem real-looking data.
  - **Trigger aktywacji jako twarda R16:** drugi wyciek PII z test data (jakikolwiek plik fixtures / test_payload / inbox_test) ALBO osobne TAK Daniela po review tego incydentu. Plus L6 z `audits/2026-05-04_R1_incident.md`.

- **R17 — Procedura kryzysowa GitHub: pre-flight checklist filter-repo** *(✅ ZATWIERDZONE 2026-05-05 jako TWARDA ZASADA — obowiązuje, trigger: osobny TAK Daniela `/yolo r15-r18` po R1 incident response)*
  - **Treść:** Każda operacja `git filter-repo` lub `git push --force` MUSI być poprzedzona pre-flight checklistą: (1) `cp -r .git ../_backup_git_<data>` — instant rollback. (2) `git bundle create ../<data>.bundle --all` — alternative state. (3) Backup workdir copies plików niezacommitowanych do `_backup_filter_repo_<data>/`. (4) Lista konkretnych wzorców do redact / paths do invert (write w plain text do replace.txt). (5) Verification BEFORE: `git log --all -p -S '<wzorzec>'` count = baseline X. (6) Wykonanie filter-repo. (7) Verification AFTER: `git log --all -p -S '<wzorzec>'` count = 0 (R12 protocol). (8) Re-add origin remote (filter-repo auto-removes). (9) Force push tylko po verification PASS. (10) Audit raport w `costsec/audits/<data>_filter_repo.md`.
  - **Dlaczego chroni biznes:** filter-repo jest nieodwracalne. Bez backup — żaden rollback nie jest możliwy. Plus filter-repo resetuje workdir do nowego HEAD — niezacommitowane zmiany w workdir mogą zostać "wymazane" (zauważone w R1 incident: zmiany w `.gitignore` sekcja 9 zostały zresetowane). Plus filter-repo auto-usuwa origin remote (safety), trzeba ręcznie re-add. Bez R17 procedura ad-hoc = wysokie ryzyko błędu w stresie incident response.
  - **Jak sprawdzamy:** Po każdym filter-repo audit raport `audits/<data>_filter_repo.md` zawiera obowiązkowe sekcje 1-10 wymienione w treści. Brak którejkolwiek = niepełen audyt. Procedura `GITHUB.md` § "Procedura kryzysowa" rozszerzona o tę checklistę.
  - **Trigger aktywacji jako twarda R17:** drugi filter-repo bez kompletnej pre-flight checklisty (np. brak backup .git) ALBO osobne TAK Daniela po review tego incydentu. Plus L7 z `audits/2026-05-04_R1_incident.md`.

- **R18 — Audit raporty incydentów: meta-protokół (sekrety strukturalnie, nie wartościowo)** *(✅ ZATWIERDZONE 2026-05-05 jako TWARDA ZASADA — obowiązuje, trigger: osobny TAK Daniela `/yolo r15-r18` po R1 incident response)*
  - **Treść:** Audit raporty incydentów (`costsec/audits/<data>_*_incident.md`) żyją w repo na trwałe — ich zawartość staje się stałym artefaktem wersji firmowej. **Sekrety opisujemy strukturalnie**, nie wartościowo. Dozwolone: typ sekretu ("hasło FTP CyberFolks user xkhvbgqqku"), status (LIVE/zrotowany), daty (utworzenia, rotacji), lokalizacja źródła ("`.claude/settings.local.json`", liczba wystąpień). NIEdozwolone: literalna wartość hasła/tokenu/klucza, nawet jeśli zrotowany/martwy. Dla **PII osób trzecich**: dozwolone uogólnione opisy klas ("klient zagraniczny — branża X", "klient PL — JST z wadium 50k"); NIEdozwolone: pełne emaile / nazwiska / telefony / adresy. Pełen forensic szczegół = `_backup_git_pre_*/` (lokalnie, gitignored, NIE w repo).
  - **Dlaczego chroni biznes:** Meta-incident 2026-05-05 — pierwsza wersja audytu R1 (`ae547ad`, force-pushed away) zawierała literalne `<REDACTED_FTP_PWD_INCIDENT_2026-05-05>` + literalny Telegram token + pełne emaile/nazwiska/telefon/adres. Hasło i token martwe (zrotowane), więc niska krytyczność dla nich, ALE PII klientów LIVE — wysoka krytyczność. Bez R18 każdy audit incydentu w przyszłości będzie potencjalnym powtórzeniem wycieku.
  - **Jak sprawdzamy:** Każdy audit raport incydentu PRZED commit przechodzi G2 skan (procedura GITHUB.md) — `grep -nE "<wzorce sekretów>"` plus `grep -nE "@(?!example\.com)|\\+48 [0-9]{3}"`. Plus checklist w nagłówku raportu: "Czy raport zawiera literalne sekrety/PII? Jeśli TAK — STOP i sanitize". Audyt monthly: review wszystkich plików w `costsec/audits/` pod kątem literalnych wartości.
  - **Trigger aktywacji jako twarda R18:** drugi meta-incident (audit z literalnymi sekretami/PII) ALBO osobne TAK Daniela po review tego incydentu. Plus L8 z `audits/2026-05-04_R1_incident.md`.
